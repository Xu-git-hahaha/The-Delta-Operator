...
def parse_log_file(filepath):
    ...
    epoch_pattern = re.compile(r"Epoch\s+(\d+)\s+Done\.\s+Avg Loss:\s+([\d\.]+)\.\s+Time:\s+([\d\.]+)s")
        ...
        for line in f:
            ...
            if epoch_match:
                epoch_data.append({
                    ...
                    'Time': float(epoch_match.group(3))
                })
...
def analyze_and_plot(filename, df_step, df_epoch):
    ...
    plt.title(f'Training Loss Curve (Manifold Projection) - {base_name}', fontsize=12, fontweight='bold')
    plt.xlabel('Training Steps')
    plt.ylabel('MSE Loss ($||x_{pred} - x_0||^2$)')
    plt.legend()
    plt.tight_layout()
    ...
    ax1.set_ylabel('Gamma (Kernel Scale $\gamma$)', color=color_gamma, fontweight='bold')
    ax1.plot(df_step['Step'], df_step['Gamma'], color=color_gamma, linewidth=2, label='Gamma ($\gamma$)')
    ...
    ax2 = ax1.twinx()  
    color_rho = '#2ca02c'  
    ax2.set_ylabel('Rho (Tail Heaviness $\\rho$)', color=color_rho, fontweight='bold')
    ax2.plot(df_step['Step'], df_step['Rho'], color=color_rho, linewidth=2, linestyle='--', label='Rho ($\\rho$)')
    ax2.tick_params(axis='y', labelcolor=color_rho)

    plt.title(f'Evolution of Geometric Kernel Parameters - {base_name}', fontsize=12, fontweight='bold')
    ...
    with open(os.path.join(output_dir, 'summary_report.txt'), 'w') as f:
        ...
        f.write(f"Gamma (Scale): Start={df_step['Gamma'].iloc[0]:.4f} -> End={df_step['Gamma'].iloc[-1]:.4f}\n")
        f.write(f"Rho (Tail):    Start={df_step['Rho'].iloc[0]:.4f}   -> End={df_step['Rho'].iloc[-1]:.4f}\n")

        f.write("\n--- Stability Statistics ---\n")
        f.write(f"Max Grad Norm: {df_step['Grad'].max():.4f}\n")
        f.write(f"Mean Grad Norm: {df_step['Grad'].mean():.4f}\n")
        if 'Grad_Smooth' in df_step.columns:
            f.write(f"Final Smoothed Grad: {df_step['Grad_Smooth'].iloc[-1]:.4f}\n")
