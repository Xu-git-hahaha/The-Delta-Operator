...
class Config:
    ...
    exp_name = "DeltaDiT_TinyImageNet_200Class"
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = os.path.join("results", f"{exp_name}_{run_id}")

    
    data_path = "data/tiny_imagenet_64x64.pt"
    image_size = 64
    patch_size = 4  
    num_classes = 200  
...
class DeltaDiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        t_feat = self.t_embedder(t_emb)
        y_feat = self.y_embedder(y)
...
class EMA:
    ...
    def update(self, model):
        ...
        model_to_avg = model.module if hasattr(model, 'module') else model
        with torch.no_grad():
            for ema_param, param in zip(self.model.parameters(), model_to_avg.parameters()):
...
def get_dataloader():
    if not os.path.exists(Config.data_path):
        raise FileNotFoundError(f"Missing {Config.data_path}")
    print(f"Loading data from {Config.data_path}...")

    
    data_dict = torch.load(Config.data_path, map_location='cpu')

    dataset = TensorDataset(data_dict['images'], data_dict['labels'])
...
class AsyncSampler(threading.Thread):
    ...
    def sample_sequence(self, model, step, device):
        ...
        y = torch.randint(0, self.config.num_classes, (n,), device=device)
        ...
        for i in range(self.config.ode_steps):
            ...
            x_pred = model(z, t_tensor, y)  

            denom = max(1 - current_t, 1e-5)
        ...
        save_path = os.path.join(self.config.output_dir, "samples", f"step_{step:06d}_labels_{y.tolist()}.png")
...
def train():
    print(f"🚀 Starting Tiny ImageNet Experiment: {Config.exp_name}")
    ...
    ema = EMA(model.module if isinstance(model, nn.DataParallel) else model, Config.ema_decay)
    ...
    for epoch in range(Config.epochs):
        ...
        for batch_idx, (x, y) in enumerate(dataloader):
            ...
            if global_step % Config.log_every_steps == 0:
                with torch.no_grad():
                    ...
                    gamma_mean = m.blocks[0].attn.gamma.mean().item()
                    rho_mean = m.blocks[0].attn.rho.mean().item()
        ...
        logger.info(f"Epoch {epoch + 1} Done. Avg Loss: {avg_loss:.6f}")
        ...
        if (epoch + 1) % Config.save_every_epochs == 0:
            torch.save(model.state_dict(), os.path.join(Config.output_dir, "checkpoints", f"epoch_{epoch + 1}.pt"))
